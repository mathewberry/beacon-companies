...
public class ListCommand {
	...
	public boolean onCommand(CommandSender sender, String[] args) 
	{
		...
		//query.getRawSql('LIMIT 17 OFFSET ' . page);
		...
		for(int count = 0; count < company.size(); count++) {
			if(company.get(count).isVisible()) {
				String owner = company.get(count).getPlayer().getUniqueId();
				map.put(count, ChatColor.AQUA + company.get(count).getName() + ChatColor.WHITE + " | " + ChatColor.AQUA + CommandHelper.getPlayerName(owner));
			}
		}
		
		CommandCore.paginate(sender, map, 1, 17);
